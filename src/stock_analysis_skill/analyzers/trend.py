# -*- coding: utf-8 -*-
"""趋势交易分析器 - 基于用户交易理念 (canonical location)"""
...
class StockTrendAnalyzer:
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        trend_score = self._TREND_SCORES.get(result.trend_status, 12)
        score += trend_score
        ...
        if math.isnan(bias) if isinstance(bias, float) else bias is None:
            bias = 0.0
        base_threshold = get_config().bias_threshold

        trend_strength = result.trend_strength
        if math.isnan(trend_strength) if isinstance(trend_strength, float) else False:
            trend_strength = 0.0
        ...
        vol_score = self._VOLUME_SCORES.get(result.volume_status, 8)
        score += vol_score
        ...
        macd_score = self._MACD_SCORES.get(result.macd_status, 5)
        score += macd_score
        ...
        rsi_score = self._RSI_SCORES.get(result.rsi_status, 5)
        score += rsi_score
