...
2. 搜索市场新闻形成复盘情报
3. 使用大模型生成每日大盘复盘报告
4. 提供 MarketSkillAnalyzer 用于 skill-first 架构的入口封装
"""
...
class MarketAnalyzer:
    ...
    def __init__(
        ...
        self.data_manager: "DataFetcherManager" = DataFetcherManager()
        self.region = region if region in ("cn", "us") else "cn"
        self.profile: MarketProfile = get_profile(self.region)
        self.strategy = get_market_strategy_blueprint(self.region)

    def _get_review_language(self, force_template: bool = False) -> str:
        configured = normalize_report_language(
            getattr(getattr(self, "config", None), "report_language", "zh")
        )
        if not force_template and self.region == "us":
            return "en"
        return configured
...
"""
    ...
    def _generate_template_review(self, overview: MarketOverview, news: List) -> str:
        ...
        template_language = self._get_review_language(force_template=True)
