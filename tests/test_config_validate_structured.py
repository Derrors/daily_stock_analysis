...
class TestValidateStructuredStockList:
    ...
    def test_missing_tushare_token_is_skill_runtime_info(self):
        cfg = _make_config(tushare_token=None)
        issues = cfg.validate_structured()
        info = next(i for i in issues if i.field == "TUSHARE_TOKEN")
        assert info.severity == "info"
        assert "主分析链" in info.message
        assert "其他数据源" not in info.message
